...
export class ZellousCore extends EventEmitter {
  ...
  broadcast(msg, exclude = null, roomId = null) {
    const data = pack(msg);
    const clients = roomId ? this.filterClientsByRoom(roomId, exclude) : Array.from(this.state.clients.values()).filter(c => c !== exclude);
  ...
  filterClientsByRoom(roomId, exclude = null) {
    return Array.from(this.state.clients.values())
      .filter(c => c.roomId === roomId && c !== exclude);
  }

  getRoom(roomId) {
    const users = this.filterClientsByRoom(roomId);
