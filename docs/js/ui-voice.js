...
const uiVoice = {
  renderGrid() {
    ...
    ui.voiceGrid.innerHTML = participants.map(p => {
      ...
      const peerKey = p._peerPubkey ? p._peerPubkey.slice(0,8) : '';
      const videoId = peerKey ? `vtile-video-${peerKey}` : '';
      const tilePubkey = p.isLocal ? state.nostrPubkey : p._peerPubkey;
      return `<div class="voice-tile${p.hasVideo ? ' has-video' : ''}" style="position:relative">
        ${p.hasVideo && videoId ? `<div class="voice-tile-video-wrap" style="position:absolute;inset:0;border-radius:8px;overflow:hidden" id="vtile-wrap-${peerKey}"></div>` : ''}
        <div class="voice-tile-avatar${spk}" style="background:${getAvatarColor(p.identity)};${p.hasVideo ? 'opacity:0;position:absolute;bottom:4px;right:4px;width:28px;height:28px;font-size:12px' : ''}">${escHtml(getInitial(p.identity))}</div>
        <div class="voice-tile-name">${escHtml(p.identity)}${qDot(p.connectionQuality)}${roleBadge(tilePubkey)}</div>
    ...
    participants.forEach(p => {
      if (!p.hasVideo || !p._videoStream || !p._peerPubkey) return;
      var wrap = document.getElementById('vtile-wrap-' + p._peerPubkey.slice(0,8));
      var existing = document.getElementById('vtile-video-' + p._peerPubkey.slice(0,8));
      if (wrap && existing) { existing.remove(); existing.style.cssText = 'width:100%;height:100%;object-fit:cover'; wrap.appendChild(existing); }
    });
  },
