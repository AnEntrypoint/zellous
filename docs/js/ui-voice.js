  ...
  renderGrid() {
    ...
    ui.voiceGrid.innerHTML = participants.map(p => {
      ...
      return `<div class="voice-tile${p.hasVideo ? ' has-video' : ''}" style="position:relative">
        ...
        <div class="voice-tile-avatar${spk}" style="background:${getAvatarColor(p.identity)};${p.hasVideo ? 'opacity:0;position:absolute;bottom:4px;right:4px;width:28px;height:28px;font-size:12px' : ''}">${getInitial(p.identity)}</div>
