...
**`docs/js/ui-chat.js` was fully dead code, now deleted** — its `_mountComposer()` targeted `.chat-input-bar`, an element the SDK's `mountCommunityApp` no longer renders (composer lives inside `.ca-app`), so `render()`/`sendChat()`/`startReply()`/`startEdit()`/`showEmojiPicker()` and its own duplicate rate-limiter were all unreachable. The real send path (`chat.js`'s `send()` via `nostr-adapter.js`) already has working rate-limiting wired to toasts. Adding a reaction to a message is still not a feature that exists in the live SDK bundle (0 occurrences of `onReact`/`toggleReaction`, and wireweave has no NIP-25 kind:7 support at all) — not a zellous wiring gap. The bundle does now render a per-message `reactions` array read-only (`.rxn` spans, no click handler) if the adapter supplies one; `nostr-adapter.js` currently doesn't, so nothing renders — building this out is a net-new protocol+SDK feature, not a fixable gap.
...
2026-07-30: gm-plugkit watcher went genuinely stuck mid-session (alive pid, frozen heartbeat, stale cached instruction/prd-resolve responses served for several dispatches before detection) -- recovered via kill+reboot, confirmed via a fresh `ts`. codesearch remains down this session (`plugin gm not loaded`, slim build has no host_vec_embed) -- same class as the already-recorded blocker, fell back to direct Read/Grep for known-path exploration. Ran a 6-agent parallel audit (error/empty-states, server/channel management, mobile breakpoints, accessibility, voice correctness, SDK adapter gaps) and found/fixed 9 real defects: silent file-upload failures (no toast), a real authz gap in `roles.js` (any admin could demote another admin or the owner), missing name-validation + last-channel-deletion guard in `channels.js`/`servers.js`, a `#drawerOverlay` with zero CSS rules (same silent-scrollHeight-inflation class as `.video-playback`), a dark-theme contrast regression (`--bg-3` broke the SDK's own 4.5:1 guarantee), 4 more SDK overlay types with the same focus-race bug already fixed in 6 others, a fully-dead legacy `ptt.js` module, and VAD (voice-activity auto-transmit) being a genuinely dead setting due to an architectural bug (the speaker analyzer ran on the same mute-gated track it needed to hear the user's own speech on) -- fixed by cloning the raw track into an always-listening local tap. Also discovered and fixed `docs/vendor/wireweave/src/*` had drifted far out of sync with the real `wireweave` sibling repo (missing 3 files entirely, missing several releases' worth of `voice.js` architecture) -- full re-vendor landed alongside the new fixes. Fixed 2 stale `C:\dev\anentrypoint-design` path references (repo actually lives at `../design`). Root-caused the vendor-drift problem itself and eliminated it at the source: switched wireweave from a hand-copied vendor directory to a live `esm.sh` CDN import of the published npm package (same pattern already used for the SDK), so it can no longer drift stale again. Reopened a stale `blockedBy:external` mobile-viewport-checks row from 2026-07-24 (the original blocker, gm's own browser verb never spawning chromium, was independently confirmed fixed upstream per this file's own historical note) and re-exercised it for real via `agent-browser` at 375x667/667x375 -- zero overflow, zero console errors, safe-area-insets confirmed already SDK-covered.
