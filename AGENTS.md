...
**SDK JS+CSS consumed LIVE from unpkg @latest (2026-07-28, supersedes the gh-pages approach)** — the inline importmap injector in `docs/nostr-chat/index.html` maps `anentrypoint-design` → `https://unpkg.com/anentrypoint-design@latest/dist/247420.js`, and the single stylesheet `<link>` points at `https://unpkg.com/anentrypoint-design@latest/dist/247420.css` (bundled: colors_and_type + app-shell + community + editor-primitives, all scoped under `.ds-247420`, matched by `<html class="ds-247420">`). zellous carries no local SDK copy and auto-tracks the SDK's npm-published `@latest` build via unpkg. To propagate an SDK change: `node scripts/build.mjs` in `C:\dev\anentrypoint-design`, commit + push (triggers `npm publish`); unpkg picks up the new version on next resolve and zellous picks it up on next load. **Tradeoff:** zellous boot now depends on `unpkg.com` being reachable; the SDK import is wrapped in try/catch and sets `window.__sdk = null` on failure (graceful-degrade, no hard crash). Note: only the bundled `247420.css` is linked — do not link individual cssPart names (`community.css`/`editor-primitives.css`) from unpkg, they are not published as separate top-level dist files.
