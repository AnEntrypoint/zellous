...
**SDK chrome uses an `Icon(name)` SVG set, not color-emoji (2026-05-28)** — anentrypoint-design components (VoiceControls, VoiceStrip, UserPanel, MobileHeader, PttButton, WebcamPreview, AppShell side-toggle) formerly used multicolor OS emoji (🎙🔊🔇📷🖥📞👥☰) as icons, clashing with the monochrome line-art design. `shell.js` now exports `Icon(name, {size})` → inline `currentColor` SVG (mic/mic-off/speaker/speaker-off/camera/screen/phone/members/menu/settings), barrel-exported, used at every voice/chrome call site. Geometric symbol glyphs (✕ ⚙ ✓ ⚠ ℹ ◻ ◉ §) are kept — they read as clean monospace symbols and are design-idiomatic. When adding chrome icons to the SDK, use `Icon()` not emoji. Built into `247420.js` (commit 3ae565a); zellous consumes live via gh-pages.

**`window.__contextMenu.show(items,x,y)` item shape is `{label, danger, disabled, icon, onSelect, separator}` — NOT `onClick`** — the SDK `C.ContextMenu` reads `{items, anchor:{x,y}, onClose}` and calls `it.onSelect`. The zellous `sdk-context-menu.js` mount previously passed `{open,x,y}` (which the component ignores), so the menu never positioned (anchored 0,0) and `open` did nothing — a silent latent bug, now fixed (mount passes `anchor:{x,y}` and gates render on `open`). Use `onSelect` for item handlers; `moderation.showMemberMenu` is the reference caller.
