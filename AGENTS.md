...
**gm's `browser` verb chromium launch was failing pre-spawn in this environment (2026-07-24), partially fixed, root cause still open** — the actual browser-launch code for the `browser` verb lives in a compiled, stripped Rust+WASM binary (`~/.gm-tools/agentplug-runner`), not `~/.gm-tools/plugkit-wasm-wrapper.js` (that JS file's distinct error-message wording and `logEvent` calls never appear in `.watcher.log`, confirming it isn't the code path actually running). Found and fixed a real environment defect along the way: the daemon's own env sets `XDG_CACHE_HOME=/config/.gmweb/cache`, and the Playwright chromium install there used a non-standard subfolder name (`chrome-linux64` instead of the universally-expected `chrome-linux`) — created the missing `chrome-linux` symlink. This did NOT fix the underlying failure: a fresh daemon reboot afterward still fails identically, and polling `ps aux` across the entire 30s timeout confirms no chromium process is ever spawned during a failing attempt — the failure is pre-spawn, in code this environment fix doesn't reach. No source repo for `agentplug-runner` is available in this workspace to root-cause further. If this recurs, the productive next step is locating and cloning that binary's actual source (not re-diagnosing the JS wrapper, which is a red herring here), not repeating the same environment-path checks.
