...
**`docs/js/ui-chat.js` was fully dead code, deleted.** Message reactions are still unbuilt: wireweave has zero NIP-25 (kind:7) support and the adapter never populates a `reactions` field, so the SDK's read-only `.rxn` span renders nothing — a net-new protocol+adapter+UI feature (spec: event shape `{kind:7, content:"+"/emoji, tags:[["e",id],["p",pubkey],["k",targetKind]]}`, subscribe via `{kinds:[7], "#e":[...]}`, aggregate client-side keyed by last `e` tag with last-write-wins per pubkey+target) — belongs in `../wireweave` first.
